...
class Registers extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('site/registers' , $data);
	...
	{	/*echo '<pre>'; print_r($_POST); die;*/
		...
		if(
			...
			$checkAlredyRegistersmobile = 	$this->common->getSingleRecordByFieldName(array('phone' =>$_POST['mobile'] , 'verify !='=>'2'), 'users');
			if (empty($checkAlredyRegistersmobile)) {
				$image = ''; 
             ...
             {
             	$output['msg'] = 'This mobile address alredy register';
             }
...
public function checkref()
	{
		...
		if((isset($_POST['ref_mobile']) && !empty($_POST['ref_mobile']))) {
			...
			if ($checkmobile) {
				$allmembership = $this->common->getAllRecords($table='membership' , 'ASC');
				$output['status'] = true;
				$output['data'] = $this->load->view('site/section/membership-forms' , array('membership_list'=> $allmembership) , true);
